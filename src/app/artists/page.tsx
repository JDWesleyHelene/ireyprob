...
export default function ArtistsPage() {
  ...
  const genres = useMemo(() => {
    ...
    artists.forEach(a => a.tags?.forEach(t => all.add(t)));
    return ["All", ...Array.from(all).sort()];
  }, [artists]);
  const filtered = activeGenre === "All" ? artists : artists.filter(a => a.tags?.some(t => t === activeGenre));

  useEffect(() => {
    const obs = new IntersectionObserver(entries => entries.forEach(e => { if(e.isIntersecting) e.target.classList.add("visible"); }), { threshold: 0.06, rootMargin: "0px 0px -5% 0px" });
      ...
      <main className="min-h-screen bg-background">
        <section className="relative pt-28 pb-8 overflow-hidden">
          ...
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16">
            ...
            <h1 className="reveal font-display text-[3rem] sm:text-[4rem] md:text-[5rem] lg:text-[6rem] font-light italic text-white leading-[0.9] tracking-tight mb-4">Our Artists</h1>
            <p className="reveal text-[14px] text-white/70 font-light max-w-lg leading-relaxed">Discover the artists represented by IREY PROD across the Indian Ocean.</p>
          </div>
        </section>
        <section className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 pb-6">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {genres.map(g => (
              <button key={g} onClick={() => setActiveGenre(g)} className={`px-4 sm:px-5 py-2 text-[11px] font-semibold tracking-[0.18em] uppercase rounded-sm border transition-all duration-300 ${activeGenre===g?"bg-foreground text-background border-foreground":"bg-transparent text-white/60 border-white/20 hover:border-white/50 hover:text-white"}`}>{g}</button>
            ))}
          </div>
        </section>
                ...
                <div key={artist.id} className={`reveal delay-${Math.min(i%3*100,300)}`}>
                  <div className="group relative overflow-hidden rounded-sm h-[380px] sm:h-[400px] spotlight-card cursor-pointer">
                    ...
                    <div className="absolute top-5 left-5 flex flex-wrap gap-1.5">
                      {artist.tags?.slice(0,2).map(t=><span key={t} className="px-2.5 py-1 bg-black/60 backdrop-blur-sm border border-white/15 rounded-sm text-[9px] font-semibold tracking-[0.2em] uppercase text-white/80">{t}</span>)}
                    </div>
