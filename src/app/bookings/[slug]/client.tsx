...
function BookingModal({ artistName, artistImage, onClose }: { artistName: string; artistImage: string; onClose: () => void }) {
  ...
  const fieldCls = (n: string) => `w-full bg-foreground/[0.03] border rounded-sm px-4 py-3 text-[13px] text-white placeholder-foreground/25 focus:outline-none transition-colors duration-300 ${errors[n as keyof FormErrors]?"border-red-500/60":"border-foreground/10 focus:border-foreground/30"}`;
      ...
      <div className="relative z-10 w-full max-w-lg bg-[#0a0a0a] border border-foreground/10 rounded-sm overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
        ...
        <div className="px-6 sm:px-8 pb-8">
          <div className="mb-6">
            ...
            <h3 className="font-display text-2xl font-extrabold italic text-foreground">Book {artistName}</h3>
          </div>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              <div>
                ...
                <input type="text" value={`Booking: ${artistName}`} readOnly className="w-full bg-foreground/[0.05] border border-foreground/10 rounded-sm px-4 py-3 text-[13px] text-foreground/50 cursor-not-allowed"/>
              </div>
              {[{name:"fullName",label:"Full Name *",placeholder:"Your full name",type:"text"},{name:"email",label:"Email *",placeholder:"your@email.com",type:"email"},{name:"address",label:"Venue / Address *",placeholder:"Event venue address",type:"text"}].map(f=>(
                <div key={f.name}>
                  ...
                  {errors[f.name as keyof FormErrors] && <p className="text-[11px] text-red-400 mt-1">{errors[f.name as keyof FormErrors]}</p>}
                </div>
              ))}
              <div>
              ...
              <div>
                ...
                <textarea name="message" value={formData.message} onChange={handleChange} rows={3} placeholder="Tell us more about your event..." className="w-full bg-foreground/[0.03] border border-foreground/10 rounded-sm px-4 py-3 text-[13px] text-white placeholder-foreground/25 focus:outline-none focus:border-foreground/30 transition-colors resize-none"/>
              </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 pb-10 sm:pb-14">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 md:gap-6">
              <div>
                ...
                <h1 className="font-display text-[2.8rem] sm:text-[4rem] md:text-[7rem] font-extrabold italic text-white leading-[0.92] tracking-tight">{artist.name}</h1>
                <div className="flex items-center gap-2 mt-3 text-white/70 text-[13px]">
                ...
                {related.map((a:any)=>(
                  <Link key={a.id} href={`/bookings/${a.slug}`} className="group relative overflow-hidden rounded-sm h-[240px]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      ...
                      <h3 className="font-display text-xl font-extrabold italic text-white group-hover:text-accent transition-colors">{a.name}</h3>
                    </div>
                  </Link>
                ))}
              </div>
