...
type Tab = "services" | "why" | "cta" | "hero";
...
export default function AdminServicesPage() {
  ...
  const save = async (extra?: Record<string,string>) => {
    ...
    const payload: Record<string,string> = {
      services_hero_heading: heroHeading,
      services_hero_sub: heroSub,
      services_why_heading: whyHeading,
      services_cta_heading: ctaHeading,
      services_cta_sub: ctaSub,
      services_cards: JSON.stringify(services),
      services_why: JSON.stringify(whyItems),
      ...extra,
    };
    try {
  ...
  const updateService = (idx:number, field:keyof Service, val:string) => {
    setServices(p => p.map((s,i) => i===idx ? {...s,[field]:val} : s));
  };
  const updateFeature = (sIdx:number, fIdx:number, val:string) => {
    setServices(p => p.map((s,i) => i===sIdx ? {...s,features:s.features.map((f,j)=>j===fIdx?val:f)} : s));
  };
  const updateWhy = (idx:number, field:"title"|"desc", val:string) => {
    setWhyItems(p => p.map((w,i) => i===idx ? {...w,[field]:val} : w));
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <span className="text-[10px] font-semibold tracking-[0.28em] uppercase text-foreground/30 block mb-1">— Pages</span>
      ...
      {tab==="hero" && (
        <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-5">
          ...
          <div>
            ...
            <input value={heroHeading} onChange={e=>setHeroHeading(e.target.value)} placeholder="What We Do" className={IC}/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Subtext</label>
            <textarea value={heroSub} onChange={e=>setHeroSub(e.target.value)} rows={3} placeholder="About the services..." className={TA}/>
          </div>
        </div>
      )}
          ...
          {services.map((s, idx) => (
            <div key={s.id} className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[10px] font-semibold tracking-[0.2em] uppercase text-accent">{s.service_number}</span>
                <h2 className="text-[13px] font-semibold text-foreground">{s.title}</h2>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Title</label>
                <input value={s.title} onChange={e=>updateService(idx,"title",e.target.value)} className={IC}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Tagline</label>
                <input value={s.tagline} onChange={e=>updateService(idx,"tagline",e.target.value)} className={IC}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Description</label>
                <textarea value={s.description} onChange={e=>updateService(idx,"description",e.target.value)} rows={3} className={TA}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Image URL</label>
                <input value={s.image} onChange={e=>updateService(idx,"image",e.target.value)} placeholder="https://..." className={IC}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Features (4 items)</label>
                <div className="grid grid-cols-2 gap-2">
                  {s.features.map((f,fi)=>(
                    <input key={fi} value={f} onChange={e=>updateFeature(idx,fi,e.target.value)} className={IC}/>
                  ))}
                </div>
              </div>
            </div>
      ...
      {tab==="why" && (
        <div className="space-y-5">
          <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-4">
            <h2 className="text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40">Section Heading</h2>
            <input value={whyHeading} onChange={e=>setWhyHeading(e.target.value)} placeholder="Why Work With Us?" className={IC}/>
          </div>
          {whyItems.map((w,idx)=>(
            <div key={idx} className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-5 space-y-3">
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Title</label>
                <input value={w.title} onChange={e=>updateWhy(idx,"title",e.target.value)} className={IC}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Description</label>
                <textarea value={w.desc} onChange={e=>updateWhy(idx,"desc",e.target.value)} rows={2} className={TA}/>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab==="cta" && (
        <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-5">
          ...
          <div>
            ...
            <input value={ctaHeading} onChange={e=>setCtaHeading(e.target.value)} className={IC}/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Subtext</label>
            <textarea value={ctaSub} onChange={e=>setCtaSub(e.target.value)} rows={3} className={TA}/>
          </div>
        </div>
      )}
    </div>
