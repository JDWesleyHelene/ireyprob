...
export default function AdminNewsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-sm text-[12px] font-medium border ${
            toast.type === 'success'
      ...
      {editing !== null && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={editing.id ? 'Edit article form' : 'New article form'}
        >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-foreground/8">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="min-h-[48px] min-w-[48px] flex items-center justify-center text-foreground/30 hover:text-foreground transition-colors"
                >
                  <svg
                    width="16"
                    height="16"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-foreground/5 border border-foreground/10 rounded-sm px-3 py-3 text-[13px] text-foreground focus:outline-none focus:border-foreground/30 min-h-[48px]"
                  >
                    <option value="draft" className="bg-[#0a0a0a]">
                      Draft
                    </option>
                    <option value="published" className="bg-[#0a0a0a]">
                      Published
                    </option>
                  </select>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  ...
                  className="flex-1 min-h-[48px] py-2.5 border border-foreground/10 text-foreground/40 text-[11px] font-semibold tracking-[0.15em] uppercase rounded-sm hover:border-foreground/20 transition-all"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  className="flex-1 min-h-[48px] py-2.5 bg-foreground text-background text-[11px] font-semibold tracking-[0.15em] uppercase rounded-sm hover:bg-accent transition-all disabled:opacity-50"
                >
          ...
          <table className="w-full min-w-[600px]" role="table" aria-label="News articles list">
            <thead>
              <tr className="border-b border-foreground/8">
                <th className="text-left px-5 py-3 text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/30">
                  Title
                </th>
                <th className="text-left px-5 py-3 text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/30 hidden sm:table-cell">
                  Status
                </th>
                <th className="text-left px-5 py-3 text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/30 hidden md:table-cell">
                  Date
                </th>
                <th className="text-right px-5 py-3 text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/30">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <tr key={i} className="border-b border-foreground/5">
                    <td className="px-5 py-4">
                      <div className="h-4 bg-foreground/5 rounded animate-pulse w-48" />
                    </td>
                    <td className="px-5 py-4 hidden sm:table-cell">
                      <div className="h-4 bg-foreground/5 rounded animate-pulse w-16" />
                    </td>
                    <td className="px-5 py-4 hidden md:table-cell">
                      <div className="h-4 bg-foreground/5 rounded animate-pulse w-24" />
                    </td>
                    <td className="px-5 py-4">
                      <div className="h-4 bg-foreground/5 rounded animate-pulse w-20 ml-auto" />
                    </td>
                  </tr>
                ))
              ) : articles.length === 0 ? (
                ...
                articles.map((article) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 hidden sm:table-cell">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-sm text-[9px] font-semibold tracking-[0.15em] uppercase border ${
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          aria-label={
                            article.status === 'published'
                              ? `Unpublish article: ${article.title}`
                          ...
                          className={`min-h-[48px] min-w-[48px] sm:min-h-0 sm:min-w-0 flex items-center justify-center px-3 py-1.5 text-[10px] font-semibold tracking-widest uppercase border rounded-sm transition-all duration-200 ${
                            article.status === 'published'
                              ? 'border-yellow-500/20 text-yellow-400/60 hover:text-yellow-400 hover:border-yellow-500/40'
