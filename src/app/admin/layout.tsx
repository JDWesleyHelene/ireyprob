...
// roles: undefined = all, ["admin","super_admin"] = admin+, ["super_admin"] = super admin only
const navItems = [
  { href:"/admin/dashboard", label:"Dashboard",    section:"content",    roles: undefined,                          icon:"M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
  { href:"/admin/artists",   label:"Artists",      section:"content",    roles: undefined,                          icon:"M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" },
  { href:"/admin/events",    label:"Events",       section:"content",    roles: undefined,                          icon:"M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
  { href:"/admin/news",      label:"News",         section:"content",    roles: undefined,                          icon:"M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" },
  { href:"/admin/bookings",  label:"Bookings",     section:"content",    roles: undefined,                          icon:"M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
  { href:"/admin/contact",   label:"Contact",      section:"content",    roles: undefined,                          icon:"M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" },
  { href:"/admin/media",     label:"Media Library",section:"content",    roles: undefined,                          icon:"M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" },
  // Admin+ only
  { href:"/admin/homepage",  label:"Homepage",     section:"content",    roles:["editor","admin","super_admin"],              icon:"M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" },
  { href:"/admin/gallery",   label:"Gallery",      section:"content",    roles:["editor","admin","super_admin"],              icon:"M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" },
  { href:"/admin/pages",     label:"Page Content", section:"content",    roles:["editor","admin","super_admin"],              icon:"M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
  // Management — admin+ only
  { href:"/admin/navigation",label:"Navigation",   section:"management", roles:["editor","admin","super_admin"],              icon:"M4 6h16M4 12h8m-8 6h16" },
  { href:"/admin/seo",       label:"SEO",          section:"management", roles:["editor","admin","super_admin"],              icon:"M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" },
  { href:"/admin/audit",     label:"Audit Log",    section:"management", roles:["editor","admin","super_admin"],              icon:"M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
  { href:"/admin/settings",  label:"Settings",     section:"management", roles:["editor","admin","super_admin"],              icon:"M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" },
  // Super Admin only
  { href:"/admin/migrate",   label:"Migrate Images",section:"management",roles:["admin","super_admin"],      icon:"M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" },
  { href:"/admin/users",     label:"Users",        section:"management", roles:["admin","super_admin"],                      icon:"M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" },
];

const mobileNavItems = navItems.slice(0, 5);

function canAccess(itemRoles: string[] | undefined, userRole: string): boolean {
...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const visibleItems    = navItems.filter(n => canAccess(n.roles, userRole));
  const contentItems    = visibleItems.filter(n => n.section === "content");
  const managementItems = visibleItems.filter(n => n.section === "management");

  const NavItem = ({ item }: { item: typeof navItems[0] }) => {
    const isActive = pathname === item.href || (item.href !== "/admin/dashboard" && pathname.startsWith(item.href + "/"));
    return (
      <Link href={item.href}
        className={`flex items-center gap-3 px-3 py-2.5 rounded-sm transition-all duration-200 ${isActive ? "bg-foreground/8 text-foreground" : "text-foreground/50 hover:text-foreground hover:bg-foreground/5"}`}>
  ...
  const currentPage = visibleItems.find(n => pathname === n.href || pathname.startsWith(n.href + "/"))?.label || "Admin";

  // Role badge
  const roleBadge = userRole === "super_admin"
    ? { label:"Super Admin", cls:"text-yellow-400/80" }
    : userRole === "admin"
    ? { label:"Admin",       cls:"text-foreground/50" }
    : { label:"Editor",      cls:"text-accent/60" };
  ...
  return (
    <div className="min-h-screen bg-[#040404] flex">
        ...
        <div className="px-4 py-4 border-t border-foreground/8">
          <div className="flex items-center gap-3 mb-3 px-1">
            ...
            <div className="min-w-0">
              <p className="text-[12px] text-foreground/70 font-medium truncate">{adminName}</p>
              <p className={`text-[9px] font-semibold tracking-widest uppercase ${roleBadge.cls}`}>{roleBadge.label}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <div className="flex-shrink-0 h-14 border-b border-foreground/8 flex items-center gap-4 px-4 sm:px-6 bg-[#040404] sticky top-0 z-30">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className={`hidden sm:block text-[9px] font-semibold tracking-widest uppercase px-2 py-0.5 rounded-sm border ${
              userRole==="super_admin" ? "border-yellow-400/20 text-yellow-400/60 bg-yellow-400/5" :
              userRole==="admin"       ? "border-foreground/15 text-foreground/40" :
                                        "border-accent/20 text-accent/50 bg-accent/5"
            }`}>{roleBadge.label}</span>
            <Link href="/" target="_blank" className="text-[10px] text-foreground/30 hover:text-foreground/60 font-mono uppercase tracking-widest transition-colors">
              View Site ↗
            </Link>
          </div>
        </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#040404] border-t border-foreground/8 flex items-center justify-around px-2 py-2">
        {mobileNavItems.filter(n => canAccess(n.roles, userRole)).map(item => {
