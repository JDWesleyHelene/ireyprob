...
export default function AdminAboutPage() {
  ...
  const [heroBg,       setHeroBg]       = useState("");   // ← FIXED: was missing
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/admin/settings", {
        ...
        body: JSON.stringify({
          about_hero_heading: heroHeading, about_hero_sub: heroSub, about_hero_bg: heroBg,
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
      ...
      {tab==="hero" && (
        <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-5">
          ...
          <div>
            ...
            <textarea value={heroSub} onChange={e=>setHeroSub(e.target.value)} rows={3} className={TA}/>
          </div>
          <ImageField label="Hero Background Image" value={heroBg} onChange={setHeroBg}
            hint="Optional. Leave empty to use the default gradient background."/>
        </div>
      )}
      ...
      {tab==="story" && (
        <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-5">
          ...
          <div>
            ...
            <input value={storyHeading} onChange={e=>setStoryHeading(e.target.value)} className={IC}/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Paragraph 1</label>
            <textarea value={storyText1} onChange={e=>setStoryText1(e.target.value)} rows={4} className={TA}/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Paragraph 2 (Mission)</label>
            <textarea value={storyText2} onChange={e=>setStoryText2(e.target.value)} rows={4} className={TA}/>
          </div>
        </div>
      )}
          ...
          {values.map((v,i)=>(
            <div key={i} className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-5 space-y-3">
              <div>
                ...
                <input value={v.title} onChange={e=>updateValue(i,"title",e.target.value)} className={IC}/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Description</label>
                <textarea value={v.desc} onChange={e=>updateValue(i,"desc",e.target.value)} rows={2} className={TA}/>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab==="cta" && (
        <div className="bg-foreground/[0.02] border border-foreground/8 rounded-sm p-6 space-y-5">
          ...
          <div>
            <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-foreground/40 mb-2">Subtext</label>
            <textarea value={ctaSub} onChange={e=>setCtaSub(e.target.value)} rows={2} className={TA}/>
          </div>
        </div>
      )}
    </div>
