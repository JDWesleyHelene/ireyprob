...
interface GImg { src: string; alt: string; id: string; }
...
export default function MasonryGallery() {
  ...
  useEffect(() => {
    fetch("/api/admin/settings").then(r=>r.ok?r.json():{}).then(d=>{
      try {
        // Use homepage_gallery_images (separate from full gallery)
        const raw = d.homepage_gallery_images || d.gallery_images;
        if (raw) {
          const imgs = JSON.parse(raw);
          if (Array.isArray(imgs) && imgs.length > 0)
            setImages(imgs.slice(0, 6)); // max 6 on homepage
        }
      } catch {}
    }).catch(()=>{});
  }, []);
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16">
        <div className="flex items-end justify-between mb-8 sm:mb-12">
          <div>
            ...
            <h2 className="font-display text-[2rem] sm:text-[2.5rem] font-extrabold italic text-foreground leading-[0.95]">Our World</h2>
          </div>
          <Link href="/gallery" className="hidden sm:inline-flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] uppercase text-foreground/40 hover:text-foreground transition-colors">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
          {[col1,col2,col3].map((col,ci) => (
            <div key={ci} className="flex flex-col gap-2 sm:gap-3">
              {col.map(img => (
                <button key={img.id} onClick={()=>setLightbox(img)} className="relative overflow-hidden rounded-sm group cursor-zoom-in block w-full">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={img.src} alt={img.alt} className="w-full h-auto object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" loading="lazy"/>
                  <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v6m3-3H7"/></svg>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="mt-6 text-center sm:hidden">
          <Link href="/gallery" className="inline-flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] uppercase text-foreground/40 hover:text-foreground transition-colors">
            View Full Gallery <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </Link>
        </div>
      </div>
      {lightbox && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4" onClick={()=>setLightbox(null)}>
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={lightbox.src} alt={lightbox.alt} className="max-w-full max-h-[90vh] object-contain rounded-sm" onClick={e=>e.stopPropagation()}/>
