...
export default function GalleryPage() {
  const [images, setImages] = useState<GImg[]>(FULL_DEFAULTS);
  ...
  useEffect(() => {
    fetch("/api/admin/settings").then(r=>r.ok?r.json():{}).then(d=>{
      try {
        const raw = d.gallery_page_images || d.gallery_images;
        if (raw) { const imgs=JSON.parse(raw); if(Array.isArray(imgs)&&imgs.length>0) setImages(imgs); }
      } catch {}
      setLoading(false);
      ...
      <main className="min-h-screen bg-background">
        ...
        <section className="relative pt-28 pb-12 overflow-hidden">
          ...
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16">
            ...
            <h1 className="font-display text-[2.2rem] sm:text-[3rem] md:text-[3.8rem] font-extrabold italic text-white leading-[0.9] tracking-tight mb-4">Our World</h1>
            <p className="text-[14px] text-white/50 font-light max-w-lg">Behind the scenes, on stage, and everywhere in between.</p>
          </div>
        </section>
